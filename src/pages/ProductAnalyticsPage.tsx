    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="bg-gradient-to-br from-card to-primary/5 border-2 border-primary/25 rounded-[20px] p-6 md:p-9 mb-5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-1.5 bg-primary/10 border border-primary/20 rounded-full px-2.5 py-[3px] text-[10px] font-bold text-primary uppercase tracking-wider mb-3.5">
            <BarChart3 className="w-3.5 h-3.5" /> {t("analytics.productAnalyticsBadge")}
          </div>
          <h1 className="font-heading text-2xl md:text-[32px] font-extrabold leading-tight mb-2.5">
            {t("analytics.productAnalyticsTitle")} <span className="text-primary">{t("analytics.productAnalyticsHighlight")}</span>
          </h1>
          <p className="text-foreground/60 text-sm max-w-[520px]">
            {t("analytics.productAnalyticsDesc")}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {usage.isLoading ? (
          ...
          <>
            <MetricCard icon="⚡" value={String(stats?.dau ?? 0)} label={t("analytics.dau")} badge={t("analytics.today")} badgeType="up" />
            <MetricCard icon="📅" value={String(stats?.wau ?? 0)} label={t("analytics.wau")} badge={t("analytics.days7")} badgeType="up" />
            <MetricCard icon="🗓️" value={String(stats?.mau ?? 0)} label={t("analytics.mau")} badge={t("analytics.days30")} badgeType="up" />
            <MetricCard icon="🪝" value={`${stats?.stickiness ?? 0}%`} label={t("analytics.stickiness")} badge="DAU/MAU" badgeType={(stats?.stickiness ?? 0) >= 20 ? "up" : "neutral"} />
          </>
        )}
      </div>

      <SectionHeader title={t("analytics.dailyActivity")} />
      <GHCard className="mb-5" headerRight={<Tag variant="green">{t("analytics.realtime")}</Tag>}>
        {usage.isLoading ? <Skeleton className="h-60" /> : (
          <ChartContainer config={usageConfig} className="h-60 w-full">
            <AreaChart data={stats?.days ?? []}>
              ...
              <Area type="monotone" dataKey="dau" stroke="hsl(var(--primary))" fill="hsl(var(--primary) / 0.18)" strokeWidth={2} name={t("analytics.dau")} />
              <Area type="monotone" dataKey="signups" stroke="hsl(var(--primary) / 0.6)" fill="hsl(var(--primary) / 0.08)" strokeWidth={2} name={t("analytics.signups")} />
            </AreaChart>
          </ChartContainer>
        )}
      </GHCard>

      <SectionHeader title={t("analytics.topActivatedTools")} />
      <GHCard headerRight={<Tag variant="blue">{t("analytics.top10")}</Tag>}>
        {tools.isLoading ? <Skeleton className="h-60" /> : !tools.data?.length ? (
          <p className="text-xs text-muted-foreground text-center py-8">{t("analytics.noActivation")}</p>
        ) : (
          <ChartContainer config={toolConfig} className="h-60 w-full">
            <BarChart data={tools.data} layout="vertical" margin={{ left: 20 }}>
              ...
              <ChartTooltip content={<ChartTooltipContent />} />
              <Bar dataKey="count" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} name={t("analytics.activations")} />
            </BarChart>
          </ChartContainer>
        )}
      </GHCard>
...
export default function ProductAnalyticsPage() {
  const { t } = useTranslation();
  return (
    <RoleGuard allowedRoles={["admin"]} fallbackMessage={t("analytics.reservedAdmin")}>
      <Analytics />
    </RoleGuard>
  );
}
