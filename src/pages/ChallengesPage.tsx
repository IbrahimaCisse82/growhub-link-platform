...
import { Trophy, Flame, Zap, Clock, CheckCircle } from "lucide-react";
...
export default function ChallengesPage() {
  ...
  usePageMeta({ title: t("challenges.title"), description: t("challenges.metaDesc") });
  const { user } = useAuth();
  ...
  const joinChallenge = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["challenge-participations"] }); toast.success(t("challenges.accept")); },
  });
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div className="bg-gradient-to-br from-card to-amber-500/5 border-2 border-amber-500/25 rounded-[20px] p-6 md:p-9 mb-5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full px-2.5 py-[3px] text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-3.5"><Trophy className="w-3 h-3" /> {t("challenges.tag")}</div>
          <h1 className="font-heading text-2xl md:text-[32px] font-extrabold leading-tight mb-2.5">{t("challenges.h1a")} <span className="text-amber-500">{t("challenges.h1b")}</span></h1>
          <p className="text-sm text-muted-foreground max-w-lg">{t("challenges.subtitle")}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-5">
        <MetricCard icon="🏆" value={String(challenges.length)} label={t("challenges.active")} badge={t("challenges.activeBadge")} badgeType="up" />
        <MetricCard icon="🔥" value={String(participations.length)} label={t("challenges.participations")} badge={t("challenges.participationsBadge")} badgeType="up" />
        <MetricCard icon="✅" value={String(completedCount)} label={t("challenges.completed")} badge={t("challenges.completedBadge")} badgeType="up" />
        <MetricCard icon="⭐" value={String(completedCount * 100)} label={t("challenges.points")} badge={t("challenges.pointsBadge")} badgeType="neutral" />
      </div>

      <h2 className="font-heading text-lg font-bold mb-3 flex items-center gap-2"><Flame className="w-5 h-5 text-amber-500" /> {t("challenges.sectionActive")}</h2>
      {challenges.length === 0 ? (
        <GHCard className="text-center py-12 mb-5">
          <Trophy className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">{t("challenges.noneActive")}</p>
          <p className="text-xs text-muted-foreground mt-1">{t("challenges.noneActiveHint")}</p>
        </GHCard>
      ) : (
              ...
              <GHCard key={challenge.id} className="relative overflow-hidden">
                ...
                {isJoined ? (
                  <div>
                    ...
                    {participation?.completed && <div className="flex items-center gap-1 text-green-500 text-[10px] font-bold"><CheckCircle className="w-3 h-3" /> {t("challenges.done")}</div>}
                  </div>
                ) : (
                  <button onClick={() => joinChallenge.mutate(challenge.id)} className="w-full bg-amber-500/10 text-amber-600 rounded-lg py-2 text-xs font-bold hover:bg-amber-500/20 transition-colors flex items-center justify-center gap-1.5">
                    <Zap className="w-3.5 h-3.5" /> {t("challenges.take")}
                  </button>
                )}
              </GHCard>
            );
      ...
      <GHCard>
        <h2 className="font-heading text-sm font-bold mb-3 flex items-center gap-2"><Trophy className="w-4 h-4 text-amber-500" /> {t("challenges.leaderboard")}</h2>
        {leaderboard.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">{t("challenges.noChallengers")}</p>
        ) : (
          <div className="space-y-2">
            {leaderboard.slice(0, 10).map((entry: any, i: number) => (
              <div key={entry.id} className="flex items-center gap-3 py-1.5">
                ...
                <div className="flex-1"><span className="text-xs font-medium">{(entry as any).profiles?.display_name ?? t("challenges.member")}</span></div>
