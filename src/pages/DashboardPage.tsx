        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-7 mt-7 pt-6 border-t border-border flex-wrap">
            {statsLoading ? (
              ...
              <>
                <div><div className="font-heading text-[26px] font-extrabold leading-none mb-[3px]">{stats?.connections ?? 0}</div><div className="text-[11px] text-muted-foreground font-medium">Connexions</div></div>
                <div><div className="font-heading text-[26px] font-extrabold leading-none mb-[3px]">{stats?.completedSessions ?? 0}</div><div className="text-[11px] text-muted-foreground font-medium">Sessions coaching</div></div>
                <div><div className="font-heading text-[26px] font-extrabold leading-none mb-[3px]">{stats?.objectivePct ?? 0}%</div><div className="text-[11px] text-muted-foreground font-medium">Objectifs atteints</div></div>
                <div><div className="font-heading text-[26px] font-extrabold leading-none mb-[3px]">{stats?.avgRating ?? "—"}<span className="text-primary text-base">★</span></div><div className="text-[11px] text-muted-foreground font-medium">NPS coaching</div></div>
              </>
            )}
          </div>
        </div>
          ...
          {!objectives || objectives.length === 0 ? (
            ...
            objectives.slice(0, 5).map((obj) => (
              <ProgressBar
                key={obj.id}
                label={obj.title}
                value={`${Math.round((obj.current_value / (obj.target_value || 1)) * 100)}%`}
                percentage={Math.round((obj.current_value / (obj.target_value || 1)) * 100)}
              />
            ))
          )}
        </GHCard>
        ...
        <div className="mt-[18px]">
          ...
          <div className="grid grid-cols-2 gap-3.5">
            {recentPosts.map((post) => (
              <GHCard key={post.id} className="cursor-pointer" onClick={() => onNavigate("feed")}>
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  <div>
                    <div className="text-xs font-bold">{post.author?.display_name ?? "Membre"}</div>
                    <div className="text-[10px] text-muted-foreground">{post.author?.company_name ?? ""}</div>
                  </div>
